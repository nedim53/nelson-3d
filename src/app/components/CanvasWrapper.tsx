...
function SceneContent({ 
  ...
  return (
    <>
      ...
      {/* Models */}
      {useSimpleModels ? (
        // Koristi jednostavne Three.js oblike
        ...
        // Koristi GLB/GLTF modele s error boundary
        Object.entries(modelUrls).map(([id, url]) => {
          if (models[id]) {
            // Determine fallback shape and color based on model ID
            const fallbackConfig = id === 'modelA' 
              ? { shape: 'box' as const, color: '#3b82f6' }
              : { shape: 'sphere' as const, color: '#ef4444' }
      ...
      {viewMode === '3d' && <OrbitControls enableDamping dampingFactor={0.05} />}
    </>
  )
}
    ...
    <div className="w-full h-screen">
      <Canvas>
        <SceneContent 
          ...
          simpleModelsConfig={simpleModelsConfig}
        />
      </Canvas>
    </div>
  )
